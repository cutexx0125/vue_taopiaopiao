<template>
  <div id="movie-detail">
    <header class="mbg pr">
      <div class="d-cover">
        <img :src="detailObj.poster ? `https://gw.alicdn.com/${detailObj.poster}` : 'https://gw.alicdn.com/tps/i1/TB147JCLFXXXXc1XVXXxGsw1VXX-112-168.png'">
      </div>
      <div class="d-info">
    ...
    <div v-if="infoObj">
      <div class="intro-block border-1px-bottom">
        <div class="d-intro">
      ...
      <div class="ab-phtot pr border-1px-bottom">
      ...
      <div class="ab-phtot crew-ab-photo pr border-1px-bottom">
        <header class="ab-header pa">
      ...
      <hot-evaluation :lists="evalList"></hot-evaluation>
    </div>
...
export default {
  ...
  created() {
    ...
    } else if (!comingLists && !hotLists.length) {
      this.$router.push('/')
    } else {
    ...
    if (detailStr) {
      this.getInfo(detailStr)
      this.getEval(detailStr)
    } else {
      this.infoObj = false
    }
    this.initSwiper()
  ...
  methods: {
    ...
    getInfo(detailStr) {
      this.pushLoadStack()
      this.$axios.get(`/movie/info/?name=${detailStr}`).then(res => {
        let data = res.data
    ...
    getEval(detailStr) {
      this.pushLoadStack()
      this.$axios.get(`/movie/evaluation/?name=${detailStr}`).then(res => {
        let data = res.data
        this.evalList = data.data.data.returnValue
        this.completeLoad()
      })
    },
    // 初始化轮播图 
